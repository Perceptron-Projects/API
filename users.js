...
app.get(
  "/api/users/:userId",
  rolesMiddleware(["admin", "branchadmin", "hr", "employee", "supervisor"]),
  async function (req, res) {
    const params = {
      TableName: EMPLOYEES_TABLE,
      Key: {
...
app.get(
  ...
  async function (req, res) {
    try {
      ...
      const { Item: branchAdmin } = await dynamoDbClient.send(
...
app.patch(
  ...
  rolesMiddleware(["admin", "branchadmin"]),
  async function (req, res) {
    ...
    const { Item: existingUser } = await dynamoDbClient.send(
      new GetCommand(params)
    );
    ...
    const updateParams = {
      ...
      UpdateExpression:
        "SET contactNo = :contactNo,branchId = :branchId, dateOfBirth = :dateOfBirth, #r = :role, email = :email, joiningDate = :joiningDate, firstName = :firstName, lastName = :lastName, username = :username, imageUrl = :imageUrl, branchName = :branchName", // Replace role with #r and add ExpressionAttributeNames
      ExpressionAttributeValues: {
    ...
    try {
      const { Attributes: updatedAttributes } = await dynamoDbClient.send(
...
app.patch(
  ...
  async function (req, res) {
    ...
    const { Item: existingBranchAdmin } = await dynamoDbClient.send(
      new GetCommand(params)
    );
...
app.get(
  ...
  async function (req, res) {
    try {
      ...
      const { Item: admin } = await dynamoDbClient.send(
        new GetCommand(adminParams)
      );
      ...
      const { Items: employeePersons } = await dynamoDbClient.send(
        new ScanCommand(employeeParams)
      );
...
app.get(
  "/api/users/branch/employees/all/:role/:branchId",
  rolesMiddleware(["branchadmin"]),
  async function (req, res) {
    try {
      ...
      const { Item: branchAdmin } = await dynamoDbClient.send(
        new GetCommand(branchAdminParams)
      );
...
app.get(
  ...
  async function (req, res) {
    try {
      ...
      const { Items: supervisorPersons } = await dynamoDbClient.send(
        new ScanCommand(supervisorParams)
      );
...
app.get(
  ...
  async function (req, res) {
    try {
      ...
      const { Items: hrPersons } = await dynamoDbClient.send(
        new ScanCommand(hrParams)
      );
...
app.get(
  ...
  async function (req, res) {
    try {
      ...
      const { Items: adminPersons } = await dynamoDbClient.send(
...
app.get(
  "/api/users/admins/:id",
  rolesMiddleware(["superadmin", "admin", "branchadmin"]),
  async function (req, res) {
    try {
...
app.get(
  "/api/users/getCompanyId/:id",
  rolesMiddleware(["superadmin", "admin", "branchadmin", "hr", "employee"]),
  async function (req, res) {
    try {
...
app.delete(
  ...
  async function (req, res) {
    try {
      ...
      const { Items: admins } = await dynamoDbClient.send(
        new ScanCommand(adminParams)
      );
...
app.get(
  ...
  async function (req, res) {
    try {
      ...
      const { Items: companies } = await dynamoDbClient.send(
...
app.get(
  "/api/users/check-email/:email/:id",
  rolesMiddleware(["superadmin", "admin", "branchadmin", "hr", "employee"]),
  async function (req, res) {
    ...
    try {
      const { Items: users } = await dynamoDbClient.send(
        new ScanCommand(empParams)
      );
      const { Items: admins } = await dynamoDbClient.send(
        new ScanCommand(adminParams)
      );
      const { Items: companies } = await dynamoDbClient.send(
        new ScanCommand(companyParams)
      );
      if (users.length > 0) {
...
app.post(
  "/api/users/company/create-branch",
  rolesMiddleware(["admin"]),
  async function (req, res) {
    const {
      branchName,
...
app.delete(
  "/api/users/company/branch/:companyId/:branchId",
  rolesMiddleware(["admin"]),
  async function (req, res) {
    try {
      const branchId = req.params.branchId;
...
app.get(
  "/api/users/company/branch-admins/:companyId",
  rolesMiddleware(["admin"]),
  async function (req, res) {
    try {
      const companyId = req.params.companyId;
      ...
      const { Items: branchAdmins } = await dynamoDbClient.send(
        new ScanCommand(params)
      );
...
app.get(
  "/api/users/company/branches/:companyId",
  rolesMiddleware(["superadmin", "admin"]),
  async function (req, res) {
    try {
...
app.patch(
  "/api/users/company/branch/:branchId",
  rolesMiddleware(["superadmin", "admin"]),
  async function (req, res) {
    try {
      const branchId = req.params.branchId;
      ...
      const updatedBranch = {
        ...
        branchImageUrl:
          imageUrl || branch.branchImageUrl || urls.companyDefaultImage,
        radiusFromCenterOfBranch:
          radiusFromCenterOfBranch || branch.radiusFromCenterOfBranch,
      };
...
app.patch(
  ...
  async function (req, res) {
    try {
      ...
      const { Item: existingAdmin } = await dynamoDbClient.send(
        new GetCommand(adminParams)
      );
      ...
      const updatedAdmin = {
        ...
        adminImageUrl:
          imageUrl || existingAdmin.adminImageUrl || urls.adminDefaultImage,
      };
      ...
      const updateParams = {
        ...
        UpdateExpression:
          "SET firstName = :firstName, lastName = :lastName, email = :email, contactNo = :contactNo, username = :username, companyId = :companyId, companyName = :companyName, adminImageUrl = :adminImageUrl",
        ExpressionAttributeValues: {
...
app.patch(
  ...
  async function (req, res) {
    try {
      ...
      const { Item: existingCompany } = await dynamoDbClient.send(
        new GetCommand(companyParams)
      );
      ...
      const updatedCompany = {
        ...
        radiusFromCenterOfCompany:
          radiusFromCenterOfCompany ||
          existingCompany.radiusFromCenterOfCompany,
        companyImageUrl:
          imageUrl || existingCompany.companyImageUrl || companyDefaultImage,
      };
      ...
      const updateParams = {
        ...
        UpdateExpression:
          "SET companyName = :companyName, #loc.#lat = :latitude, companyEmail = :companyEmail, contactNo = :contactNo, #loc.#long = :longitude, radiusFromCenterOfCompany = :radiusFromCenterOfCompany, companyImageUrl = :companyImageUrl",
        ExpressionAttributeNames: {
          "#loc": "location",
          "#lat": "latitude",
          "#long": "longitude",
        ...
        ExpressionAttributeValues: {
          ...
          ":radiusFromCenterOfCompany":
            updatedCompany.radiusFromCenterOfCompany,
...
function generateUniqueUserId() {
  ...
  const shortId = Buffer.from(uuid)
    .toString("base64")
    .replace(/=/g, "")
    .slice(0, 5); // Convert UUID to Base64 and truncate
  return shortId;
}
...
app.post("/api/users/forget-password", async function (req, res) {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Password reset token sent successfully via email" });
  } catch (error) {
    console.error("Error:", error);
    res
      .status(500)
  ...
  try {
    ...
    await dynamoDbClient.send(
      new UpdateCommand({
        ...
        Key: { userId },
        UpdateExpression:
          "SET password = :password, otp = :nullValue, expiresAt = :pastTime",
        ExpressionAttributeValues: {
          ":password": hashedPassword,
          ":nullValue": null,
          ":pastTime": 0,
